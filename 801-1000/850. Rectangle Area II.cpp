// https://leetcode.com/problems/rectangle-area-ii/discuss/1419388/Rectangle-Area-II-oror-Line-Sweep-oror-O(N2)-oror-C%2B%2B-or-Python
class Solution {
public:
        ...
        for (auto y: ys)
        {
            ...
            for (auto const & r: rectangles)
            {
                ...
                if (r[1] <= prev_y && r[3] >=y)
                {
                    if (r[0] > x_end)
                    {
                        res+= height * (x_end-x_start) % mod; // meeting room
                        x_start = r[0];
                    }
                    if (r[2] > x_end)
                    {
